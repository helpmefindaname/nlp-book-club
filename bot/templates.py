...
date_poll_regex = re.compile(r"\*\s+(?P<emoji>(?:" + "|".join(map(re.escape, number_emojis)) + "))\s+(?P<date>\d\d\d\d-\d\d-\d\d)")


def get_date_poll_message_text(dates: list[datetime], emojis: list[str]) -> str:
    selections = "\n".join(f"* {emoji} {dt.year:04}-{dt.month:02}-{dt.day:02} 19:00" for dt, emoji in zip(dates, emojis))
